document.addEventListener("DOMContentLoaded", function () {
  ...
  function atualizarEstadosDinamicamente() {
    const ocorrencias = JSON.parse(localStorage.getItem("ocorrenciasLista") || "[]");
    const segurancasPorOcorrencia = JSON.parse(localStorage.getItem("ocorrenciasSegurancas") || "{}");
    const estadosPorOcorrencia = JSON.parse(localStorage.getItem("ocorrenciasEstados") || "{}");

    const emAcao = new Set();

    for (const designacao in segurancasPorOcorrencia) {
      const estado = estadosPorOcorrencia[designacao];
      if (estado === "Em análise") {
        const lista = segurancasPorOcorrencia[designacao];
        lista.split(";").map(n => n.trim()).forEach(nome => {
          if (nome) emAcao.add(nome);
        });
      }
    }

    listaDePeritos.forEach(perito => {
      if (emAcao.has(perito.nome)) {
        perito.estado = "em_acao";
      } else if (perito.estado !== "indisponivel") {
        perito.estado = "disponivel";
      }
    });

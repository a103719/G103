...
const materiais = [
  { id: 1, nome: "Cone de sinalização", stock: 40, unidade: "un" },
  { id: 2, nome: "Semáforo portátil", stock: 10, unidade: "un" },
  { id: 3, nome: "Extintor CO2", stock: 20, unidade: "un" }
];
...
function eliminarLinha(icone) {
  ...
  // 1) repõe stock de cada material usado
  const materiaisJSON = linha.getAttribute("data-materiais") || "[]";
  ...
  // 2) guarda o stock reposto no localStorage
  localStorage.setItem("materiaisStock", JSON.stringify(materiais));

  // 3) atualiza o texto de todas as opções do <select>
  materiais.forEach(m => atualizarTextoSelect(m.id));

  // 4) remove a linha da tabela
  linha.remove();

  // 5) regrava a lista de auditorias (sem esta) no localStorage
  guardarAuditoriasNaStorage();
}
